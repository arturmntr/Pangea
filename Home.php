...
<body>
	...
	<div class="top_menu"id="menu_login" align="center">
		...
		<form action="login.php" class="menu" align="center" method="post">
			...
			<?php if(isset($_SESSION['unauthenticated'])): ?>
	...
	<div class="caixa">
		...
		<table class="caixa_image">
			<tbody>
				<!--tr>
					<td class="full_empty"></td>
				</tr-->
				<tr>
					<!-- <td class="caixa_empty"></td> -->
					<td class="caixa_main_center" colspan="2">
						<div class="main_logo">
							<div class="sub_logo">Pangea</div>
						</div>
					</td>
				</tr>
				<tr>
					<!-- <td class="caixa_empty"></td> -->
					<td class="caix" colspan="2">
						<div class="caixa_button white notwrap limit login">Começar à Aprender Agora</div>
					</td>
				</tr>
				<tr>
					<td class="caix" colspan="2">
						<div class="caixa_language"></div>
					</td>
				</tr>
				<tr>
					<td class="barra_status language"><i class="material-icons white">keyboard_arrow_down</i></td>
				</tr>
	...
	<script>
		$('.login').click ( function addObs(){
			...
			document.getElementById("menu_login").classList.add("showing");
		} );
		function hideOverlay(){
			document.getElementById("overlay_dark").style.display = "none";
			//document.getElementById("menu_login").style.display = "none";
			document.getElementById("menu_login").classList.remove("showing");
		}
	</script>
	<script type="text/javascript" src="who.js"></script>
	<script>
		function sendLogin(){
			/*var call_user = document.getElementById("entre_user").value;
			var call_pass = document.getElementById("entre_pass").value;
			var found_me = false;
			for(var i = 0; i < obj.users.length; i++){
				console.log( obj.users[i].ID + ", " + call_user );
				if( obj.users[i].ID == call_user && obj.users[i].Password == call_pass ){
					console.log( "Entrando como " + obj.users[i].Name + ": " + call_user );
					document.getElementById("user_not_found").classList.remove("errored");
					found_me = true;
					who_name = obj.users[i].Name;
					window.location.href = 'Estudo.html';
					break;
				}
			}
			if(!found_me){
				console.log("Login Try Failed.");
				document.getElementById("user_not_found").classList.add("errored");
			}*/
		}
	</script>
	<script>
	var text = '{"users":[' +
		'{"ID":"novo.aluno" ,"Name":"João de Exemplo da Silva","Password":"123456","Search":false },' +
		'{"ID":"santana" ,"Name":"Maria Santana","Password":"123456","Search":true },' +
		'{"ID":"bahia" ,"Name":"Maria Placeholder Souza","Password":"123456" }]}';
		obj = JSON.parse(text);
	</script>
</body>
</html>